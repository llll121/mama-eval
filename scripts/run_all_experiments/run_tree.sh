...
# Run the MAMA multi-agent experiments on the tree topology
mkdir -p logs
...
for graph_type in "${graph_types[@]}"; do
  for num_agents in "${num_agents_list[@]}"; do
    case "$num_agents" in
      4)
        target_attacker_pairs=("0,1" "2,0" "2,1" "2,3")
        ;;
      5)
        target_attacker_pairs=("2,3" "2,4" "3,0" "3,2" "4,2" "4,3")
        ;;
      6)
        target_attacker_pairs=("1,5" "3,1" "3,0" "4,5" "4,1" "5,0" "5,2" "5,3")
        ;;
      *)
